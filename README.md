...
```js
// sw.js の先頭
var CACHE = 'plank-timer-v6';   // ← 変更のたびに v7, v8 … と上げる
```
...
- **全セット終了では鳴らさない。**3・2・1 のあと、いつもは開始音が続くところに
  何も来ない。その静けさがそのまま「終わった」の合図になる。
  ゴングを鳴らさないアプリなので、最後だけ大きな音を出すのは筋が通らない
- **数字は声のまま。**「残り10秒」は音では伝えられない。ここだけ `speechSynthesis` を使う。
...
だから `speakNow` では cancel しない。喋らせるのは「残り10秒」1種類だけなので、
溜まって打ち消し合うこともない。
